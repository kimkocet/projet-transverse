...
public class Plateau implements Cloneable {
				...
				if (!jeuTermine(j1, ia)) {
					...
					if (!jeuTermine(j1, ia)) {
						do {
							do {
								System.out
										.println("entrer la ligne suivie de la colonne du coup à jouer (joueur 1) : ");
								l = sc.nextInt();
								c = sc.nextInt();
							} while (l < 0 || l > 3 || c < 0 || c > 3);
							coup = j1.jouerCoup(l, c, this);
	...
	public boolean jeuTermine(Joueur j1, Joueur j2) {
		...
		System.out.println(j2.getNumero());
		System.out.println("hello");
		for (int i = 0; i < SIZE; i++) {
			for (int col = 0; col < 2; col++) {
				for (int j = 0; j < 3; j++) {
					System.out.println(plateau[i][col + j]);	
					if (plateau[i][col + j] == j1.getNumero()) {
						compteurJ1++;
		...
		for (int j = 0; j < SIZE; j++) {
			...
			for (int ligne = 0; ligne < 2; ligne++) {
				...
				for (int i = 0; i < 3; i++) {
					...
					/*
					 * else if(plateau[i+ligne][j] == j2.getNumero()) { //ERREUR compteurJ2++; }
					 */
				}
				if (compteurJ1 == 3) {
					System.out.println("Le joueur 1 a gagné !");
		...
		for (int deb = 0; deb < 2; deb++) {
			for (int i = 0; i < 3; i++) {
				...
				/*
				 * if(plateau[deb+i][deb+i] == j2.getNumero()) { //ERREUR compteurJ2++; }
				 */
			}
			if (compteurJ1 == 3) {
				System.out.println("Le joueur 1 a gagné !");
		...
		for (int deb = 0; deb < 2; deb++) {
			for (int i = 0; i < 3; i++) {
				...
				/*
				 * if(plateau[deb+i][SIZE-(deb+i)-1] == j2.getNumero()) { //ERREUR compteurJ2++;
				 * }
				 */
			}
			if (compteurJ1 == 3) {
		...
		for (int i = 1; i < 4; i++) {
			...
			/*
			 * if(plateau[i][i-1] == j2.getNumero()) { //ERREUR compteurJ2++; }
			 */
		}
		if (compteurJ1 == 3) {
			System.out.println("Le joueur 1 a gagné !");
		...
		for (int i = 1; i < 4; i++) {
			...
			/*
			 * if(plateau[i][SIZE-i] == j2.getNumero()) { compteurJ2++; }
			 */
		}
		if (compteurJ1 == 3) {
			System.out.println("Le joueur 1 a gagné !");
		...
		for (int i = 0; i < 3; i++) {
			...
			/*
			 * if(plateau[i][SIZE-2-i] == j2.getNumero()) { //ERREUR compteurJ2++; }
			 */
		}
		if (compteurJ1 == 3) {
			System.out.println("Le joueur 1 a gagné !");
